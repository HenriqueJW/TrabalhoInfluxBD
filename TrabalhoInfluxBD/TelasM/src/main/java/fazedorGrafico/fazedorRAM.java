 ...
 * @author Micro
 */
public class fazedorRAM {
    
    
    public XYDataset createDataSet(List<List<Double>> listaRegistro) {

        XYSeries series = new XYSeries("Uso de RAM em GigaBytes");

        int tempo = 1;
        for (List l : listaRegistro) {

            series.add(tempo, (double) l.get(1));
            tempo++;
        }
    ...
    public JFreeChart createChart(XYDataset dataSet) {
        ...
        JFreeChart grafico = ChartFactory.createXYLineChart("Uso da RAM ao longo do tempo",
                "Dias",
                "Uso RAM (GB)",
                dataSet,
